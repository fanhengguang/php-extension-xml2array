...
if test "$PHP_XML2ARRAY" != "no"; then
  ...
  dnl [
  dnl   PHP_ADD_LIBRARY_WITH_PATH($LIBNAME, $XML2ARRAY_DIR/lib, XML2ARRAY_SHARED_LIBADD)
  dnl   AC_DEFINE(HAVE_XML2ARRAYLIB,1,[ ])
  dnl ],[
  dnl   AC_MSG_ERROR([wrong xml2array lib version or lib not found])
  dnl ],[
  dnl   -L$XML2ARRAY_DIR/lib -lm
  dnl ])
  dnl
  dnl PHP_SUBST(XML2ARRAY_SHARED_LIBADD)

  PHP_NEW_EXTENSION(xml2array, xml2array.c, $ext_shared)
fi
